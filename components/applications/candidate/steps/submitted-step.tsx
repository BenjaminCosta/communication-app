...
export function SubmittedStep({ application }: { application: CandidateApplication }) {
  const approved = application.status === "approved" || application.status === "agreement_pending"
  const agreementExpired = application.agreement.status === "expired"
  return (
    <div className="min-h-0 flex-1 overflow-y-auto scrollbar-hide">
      <div className="applications-step-enter mx-auto flex w-full max-w-xl flex-col items-center px-5 pb-10 pt-[clamp(2.5rem,10vh,5rem)] text-center md:px-6">
        ...
        <p className="mt-2 max-w-sm text-[0.9375rem] leading-relaxed text-[var(--apps-text-muted)]">
          {approved ? (
            agreementExpired ? (
              "Your agreement signing window has expired. Please contact SVC for a new secure link."
            ) : (
              "Your operating agreement is ready. Please use the secure signing link sent by SVC to continue your onboarding."
            )
          ) : (
            <>
        ...
        <AppsCard className="mt-7 w-full p-5 text-left">
          ...
          <ol className="mt-3 flex flex-col gap-3">
            {(approved
              ? [
                  "Open the secure agreement link sent by SVC.",
                  "Read and sign the operating agreement before the link expires.",
                  "After signing, SVC will finish payroll and onboarding.",
                ]
              : [
