...
/**
 * AI transcription pipeline state, independent of `IntroVideoState` (which is
 * only about the upload). `null` means no video has ever been uploaded — the
 * candidate is never blocked on this transitioning past "pending".
 */
export type TranscriptionStatus = "pending" | "processing" | "completed" | "failed"

export const INTRO_VIDEO_MISSING_FIELDS = ["name", "town", "yearsOfExperience"] as const
export type IntroVideoMissingField = (typeof INTRO_VIDEO_MISSING_FIELDS)[number]

/**
 * Structured extraction from the intro video transcript. Internal reviewer
 * tool only — never used to auto-fill or gate the candidate's application,
 * and never scores the candidate (personality, accent, appearance, fit).
 */
export interface VideoSummary {
  ...
  summary: string
  missingFields: IntroVideoMissingField[]
  /** True when the model wasn't confident it captured all three fields. */
  needsReview: boolean
}
...
export interface IntroVideo {
  ...
  /** Candidate-driven: set to "pending" on upload. Everything past that is server-owned. */
  ...
  /** The storagePath that `transcript`/`summary` actually correspond to — lets the UI notice a re-upload the pipeline hasn't caught up with yet. */
...
export const INTRO_VIDEO_MAX_DURATION_SECONDS = 120
...
export interface TranscriptionStatusMeta {
  label: string
  tone: ApplicationTone
}
 ...
 */
export function describeTranscriptionStatus(status: TranscriptionStatus | null): TranscriptionStatusMeta {
  switch (status) {
    case "pending":
