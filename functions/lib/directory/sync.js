...
function relationDocId(fromId, toId) {
 ...
 */
async function upsertRelationEdge(db, input) {
    ...
    if (!snap.exists)
        patch.createdAt = firestore_1.FieldValue.serverTimestamp();
 ...
 */
async function deactivateStaleSyncEdges(db, entityDirId, otherPrefix, keepIds) {
    ...
    for (const relDoc of snap.docs) {
        ...
        if (data.source !== RELATION_SYNC_SOURCE)
            continue;
        ...
        if (!other.startsWith(otherPrefix))
            continue;
        if (keepIds.has(other))
            continue;
        batch.update(relDoc.ref, { active: false, updatedAt: firestore_1.FieldValue.serverTimestamp() });
        any = true;
    }
    if (any)
        await batch.commit();
 ...
 */
async function syncPersonCompanyRelation(db, personEntry) {
 ...
 */
async function syncInvolvedPeopleRelations(db, entry, data) {
    ...
    for (const person of involvedPeople) {
        ...
        if (!rawId)
            continue;
        const personDirId = (0, directory_core_1.directoryId)("person", rawId);
...
async function syncJobCompanyRelation(db, jobEntry) {
...
exports.syncDirectoryOnContactWrite = functionsV1.firestore
    ...
    if (!change.after.exists) {
        ...
        await deactivateStaleSyncEdges(db, (0, directory_core_1.directoryId)("person", id), "", new Set());
