...
- Admin-only cleanup for bad data: merging a duplicate contact into a survivor (including reassigning its historical Communications references), and deleting a person, company, or job outright.
...
- ~~Complete duplicate merging, including safe reassignment of historical references in Communications.~~ Done for people (the common case): admins can merge duplicate contacts from the profile screen, including message re-pointing in Communications — see `docs/svc-directory-ui-context.md` §14. Company/job merge is intentionally out of scope; flag those for review instead.
