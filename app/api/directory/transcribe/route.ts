...
import { transcribeDirectoryAudio } from "@/features/directory/ai/server/transcription-service"
...
export async function POST(request: Request): Promise<Response> {
  ...
  try {
    ...
    if (Number.isFinite(contentLength) && contentLength > DIRECTORY_AI_LIMITS.maxAudioBytes + 256 * 1024) {
    ...
    if (audio.size > DIRECTORY_AI_LIMITS.maxAudioBytes) {
      throw new AiError("payload-too-large", "The recording file is too large.")
    }
    ...
    const validatedAudio = await validateOutlookAudio(audio, reportedDurationMs, DIRECTORY_AI_LIMITS.maxAudioSeconds)
    audioMetadata = {
      bytes: audio.size,
    ...
    const result = await transcribeDirectoryAudio(
      { file: audio, fileName: audio.name || "directory-question.webm", language },
