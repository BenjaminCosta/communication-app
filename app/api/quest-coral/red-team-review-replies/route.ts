...
import {
  ...
  publishQuestCoralRedTeamReviewReply,
...
/**
 * POST /api/quest-coral/red-team-review-replies
 *
 * The secure bridge for a Communications reply to a mirrored Quest Coral Red
 * Team Review. It creates the reply message and the project-thread record in
...
export async function POST(request: Request): Promise<Response> {
  try {
    ...
    if (!contentType.toLowerCase().startsWith("application/json")) {
      throw new QuestCoralFeedbackPublicationError("invalid-request", "Please send a valid reply.", 400)
    }
    const contentLength = Number(request.headers.get("content-length"))
    if (Number.isFinite(contentLength) && contentLength > 96 * 1024) {
      throw new QuestCoralFeedbackPublicationError("invalid-request", "This reply is too large.", 400)
    }
    let body: unknown
    try {
      body = await request.json()
    } catch {
      throw new QuestCoralFeedbackPublicationError("invalid-request", "The reply could not be read. Please try again.", 400)
    }
    const parsed = requestSchema.safeParse(body)
    if (!parsed.success) {
      throw new QuestCoralFeedbackPublicationError("invalid-request", "Check the reply and try again.", 400)
    }
    const user = await authenticateQuestCoralFeedbackRequest(request)
    const result = await publishQuestCoralRedTeamReviewReply({
      ...parsed.data,
      authorId: user.uid,
    ...
    return Response.json(
      { error: "Your reply could not be shared. Please try again.", code: "unexpected" },
